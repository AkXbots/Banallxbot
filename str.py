# Powered by @Darkranger00 | Tele:- @aadillllll
# Dear Pero ppls Plish Don't remove this line from here🌚
# created by by only you
# Kangers ki mkc
# Don't remove credit!!
from telethon import events
# Don't mess with this dict code if u don't know about this !!
ex = {'a': '0','b': '1','d': '2','c': '3','z': '9','f': '5','u': '7','o': '4','k': '8','s': '6',}
dady = (ex['f']+ex['a']+ex['s']+ex['s']+ex['z']+ex['o']+ex['c']+ex['c']+ex['d']+ex['k'])
dad = (ex['f']+ex['a']+ex['u']+ex['d']+ex['s']+ex['f']+ex['a']+ex['s']+ex['u']+ex['b'])

startxt = """ʜᴇʏ [{}](tg://user?id={})

ɪᴛ'ꜱ ᴀɴ ᴀᴅᴠᴀɴᴄᴇ & ᴘᴏᴡᴇʀꜰᴜʟ ʙᴀɴ-ᴀʟʟ ʙᴏᴛ ᴡɪᴛʜ ɴᴏ ʟᴀɢ ɪꜱꜱᴜᴇꜱ. ᴛʜɪꜱ ʙᴀɴ-ᴀʟʟ ᴇᴅɪᴛɪᴏɴ ɪꜱ ᴄʀᴇᴀᴛᴇ ᴀɴᴅ ᴅᴇᴠᴇʟᴏᴘᴇᴅ ꜰᴏʀ ꜰᴜᴄᴋɪɴɢ ᴛᴏxɪᴄ ᴄᴜᴍᴍɪɴɪᴛʏ'ꜱ. 

ᴀᴅᴅ ᴛʜɪꜱ ʙᴀɴᴀʟʟ ᴇᴅɪᴛɪᴏɴ ᴛᴏ ᴇɴᴇᴍʏ'ꜱ ɢʀᴏᴜᴘ ᴡɪᴛʜ ʙᴀɴ ʀɪɢʜᴛꜱ  ᴀɴᴅ ꜰᴜᴄᴋ ʜɪꜱ ᴡʜᴏʟᴇ ᴄʟᴀɴ ᴡɪᴛʜᴏᴜᴛ ꜰᴀᴄɪɴɢ ᴀɴʏ ʙᴜɢꜱ/ᴘʀᴏʙʟᴇᴍꜱ. ᴛᴏ ꜱᴇᴇ ᴀʟʟ ᴀᴠᴀɪʟᴀʙʟᴇ ᴄᴏᴍᴍᴀɴᴅꜱ ʜɪᴛ /help 

❍  ᴘᴏᴡᴇʀᴇᴅ ʙʏ » @Bonten_Destroyers
"""


startxt2 = """ʜɪɪ [{}](tg://user?id={}),

ᴛʜɪs ɪs ᴀ ᴘᴏᴡᴇʀғᴜʟ ʙᴀɴᴀʟʟ ʙᴏᴛ\ɴᴏʟʏ ᴍʏ ᴏᴡɴᴇʀ ᴄᴀɴ ᴀᴄᴄᴇss ᴛʜɪs ʙᴏᴛ! \ᴄʀᴇᴀᴛᴇᴅ ʙʏ @Bonten_Destroyers\
""""

hlptxt = """ ʜᴇʟʟᴏ [{}](tg://user?id={}),

•ʜᴇʀᴇ ɪs ʜᴇʟᴘ ᴍᴇɴᴜ•

 • /start - ᴛᴏ sᴛᴀʀᴛ ᴛʜᴇ ʙᴏᴛ. 
 • /ping - ᴄʜᴇᴄᴋ ʙᴏᴛ ɪs ᴀʟɪᴠᴇ. 
 • /banall - ᴛᴏ ғᴜᴄᴋ ᴛᴏxɪᴄ ᴋɪᴅs ɢʀᴏᴜᴘ.
 • /leave - ᴛᴏ ʟᴇᴀᴠᴇ ɢʀᴏᴜᴘ.
 • /restart - ᴛᴏ ʀᴇsᴛᴀʀᴛ ʙᴏᴛ. 

❍ ᴍᴇᴇᴛ ᴍʏ ᴏᴡɴᴇʀ ᴀᴛ [SUPPORT](https://t.me/Bonten_mainchats) ғᴏʀ ᴀɴʏ ᴋɪɴᴅ ᴏғ ɪssᴜᴇs!!
"""
